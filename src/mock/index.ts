...
// 精确查找 /event/1
// Mock.mock(/\/(\w+)\/([^?]+)$/, 'get', (options: MockRequest): MockResponse<object> => {
//     const urlReg = new RegExp(/\/(\w+)\/([^?]+)$/);
//     const res = urlReg.exec(options.url);
//     let recordId = '';
//     let type = '';
//     if (res.length) {
//         type = res[1];
...
// // 修改 /event/1
// Mock.mock(/\/(\w+)\/[^?]+$/, 'put', (options: MockRequest) => {
//     const urlReg = new RegExp(/\/(\w+)\/([^?]+)$/);
//     const res = urlReg.exec(options.url);
//     let recordId = '';
//     let type = '';
...
//     const index = db.find((item) => item.id === recordId && item.type === type);
//     const record = Object.assign(db[index], options.body);
...
// // 删除 /event/1
// Mock.mock(/\/(\w+)\/[^?]+$/, 'delete', (options: MockRequest) => {
//     const urlReg = new RegExp(/\/(\w+)\/([^?]+)$/);
//     const res = urlReg.exec(options.url);
//     let recordId = '';
//     let type = '';
...
// // 查找一组记录 /event/?creator=10000&limit=20&offset=0
// Mock.mock(/\/(\w+)\/\?.*/, 'get', (options: MockRequest): MockResponse<object[]> => {
//     const urlReg = new RegExp(/\/(\w+)\/\?.*/);
//     const res = urlReg.exec(options.url);
//     let type = null;
